% Driver: add the MATLAB/ port folder to the path and run the pipelines.
%
% All port functions live flat in MATLAB/, so a single non-recursive addpath of
% that folder is enough. Do NOT use addpath(genpath(repo_root)) — that recursively
% adds the whole repo (.git/, results/, data/, ...), which is slow, pollutes the
% path, and triggers "shadows a built-in" warnings from unrelated folders.
