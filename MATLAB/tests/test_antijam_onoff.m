...
% ...and FINITE. With the onoff block absent the graded-release fields are NaN,
% and a missing NaN guard let that propagate into the diagonal loading and turn
% the beamformer solve singular -- silently, because the run still completed.
verifyTrue(testCase, all(isfinite(a.sinr_db)), ...
    'An un-repaired run must produce finite SINR at every step.');
verifyTrue(testCase, all(isfinite(a.W(:))), ...
    'An un-repaired run must produce finite weights at every step.');
