...
fprintf('\n[parser] E_complex size = %s ; n_theta=%d n_phi=%d\n', ...
        mat2str(size(p.E_complex)), ref.n_theta, ref.n_phi);

verifySize(testCase, p.E_complex, [ref.n_theta, ref.n_phi]);
...
for i = 1:numel(ref.samples)
    ...
    verifyEqual(testCase, p.copol_abs(t, q),   s.copol_abs,   'RelTol', 1e-9, msg);
    verifyEqual(testCase, p.copol_phase(t, q), s.copol_phase, 'RelTol', 1e-9, msg);
    verifyEqual(testCase, p.cross_abs(t, q),   s.cross_abs,   'RelTol', 1e-9, msg);
    verifyEqual(testCase, p.cross_phase(t, q), s.cross_phase, 'RelTol', 1e-9, msg);
    verifyEqual(testCase, real(p.E_complex(t, q)), s.Ecx_real, 'AbsTol', 1e-9, msg);
    verifyEqual(testCase, imag(p.E_complex(t, q)), s.Ecx_imag, 'AbsTol', 1e-9, msg);
    verifyEqual(testCase, real(p.cross_complex(t, q)), s.Xcx_real, 'AbsTol', 1e-9, msg);
    verifyEqual(testCase, imag(p.cross_complex(t, q)), s.Xcx_imag, 'AbsTol', 1e-9, msg);
end
end
...
col0 = p.E_complex(:, 1);     % all theta at phi index 1
row0 = p.E_complex(1, :).';   % all phi at theta index 1 (as column)
...
sz = size(patterns(1).E_complex);
for i = 2:numel(patterns)
    verifyEqual(testCase, size(patterns(i).E_complex), sz);
end

% Element [1] (first by index) matches the standalone parse of that file.
p1 = parse_cst_file(testCase.TestData.oneFile);
verifyEqual(testCase, sum(patterns(1).copol_abs(:)), ref.copol_abs_sum, 'RelTol', 1e-9);
verifyEqual(testCase, patterns(1).E_complex, p1.E_complex, 'AbsTol', 1e-12);
end
